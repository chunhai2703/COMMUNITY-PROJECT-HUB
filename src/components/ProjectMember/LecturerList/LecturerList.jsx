...
export const LecturerList = (props) => {
  ...
  const fetchAllLecturer = useCallback(async () => {
    setIsLoading(true);
    const response = await GetAllLecturerOfProject(
      projectId,
      searchValue,
      pageNumber,
      rowsPerPage
  ...
  const onConfirmDelete = async () => {
    ...
    setIsLoading(true);
    const response = await RemoveLecturerFromClass(
      selectedLecturer.lecturerId,
      selectedLecturer.classId
    );
    const responseData = await response.json();
    if (response.ok) {
  ...
  const getMenuItems = (lecturer) => [
    {
      ...
      label: (
        <button
          ...
          onClick={() => handleDetailOpen(lecturer)}
        >
      ...
      ? [
          {
            ...
            label: (
              <button
                ...
                onClick={() => handleDeleteOpen(lecturer)}
              >
            ...
            >
              <div className="flex">
                  ...
                  <Avatar
                    src={
                      selectedLecturer.avatar ? (
                        <img src={selectedLecturer?.avatar} alt="avatar" />
                      ) : null
                    }
                    style={{
                      backgroundColor: selectedLecturer.avatar
                        ? ""
                        : avatarBackground,
                ...
                <div className="w-1/2" style={{ flex: 1 }}>
                  <TextField
                    ...
                    label="ID"
                    fullWidth
                    value={selectedLecturer.accountCode}
                    variant="outlined"
                  />
                  <TextField
                    label="Họ và tên"
              ...
              <div className="flex mt-2">
                <TextField
                  ...
                  label="Ngày sinh"
                  fullWidth
                  value={formatDate(selectedLecturer.birthdate)}
                  variant="outlined"
                />
                <TextField
                  label="Giới tính"
                  fullWidth
                  value={selectedLecturer.gender}
                  variant="outlined"
                />
              </div>
            </div>
      ...
      <Dialog open={openDelete} onClose={handleDeleteClose}>
        ...
        <DialogContent>
          <p className="pt-3 pb-3">
            Bạn có chắc muốn xóa giảng viên này ra khỏi dự án?
          </p>
        </DialogContent>
        <DialogActions>
