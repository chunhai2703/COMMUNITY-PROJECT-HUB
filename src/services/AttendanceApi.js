...
export async function getAllTraineesAttendance(classId) {
  try {
    const response = await fetch(`${baseUrl}/api/Attendance/class-attendance?classId=${classId}`, {
      method: 'GET',
      headers: {
...
export async function importAttendanceTrainee(classId, formData) {
  ...
  try {
    const response = await fetch(`${baseUrl}/api/Attendance/import-attendance-file?classId=${classId}`, {
      method: 'POST',
      headers: {
        "Authorization": `Bearer ${localStorage.getItem("accessToken")}`
      },
